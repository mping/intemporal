...
Every stub op calls `p/find-event` **on the store**, not on the pass-local history already in the context atom (20 call sites in `core.cljc`, plus `run-once` in each engine — 21 in-body sites in total; the "14" first reported here was an undercount). On JDBC that's a `SELECT` per step (`jdbc.clj:176-181`); on FDB `find-event` **loads the entire history** (`fdb.clj:124-129`). Each loop iteration reloads full history (`execution.clj:667`); `check-cancelled!` does a store read per seq op (`context.cljc:99-105`); finalization does 2 more full history loads (`parent-link` `execution.clj:347-349`, `has-children?` `execution.clj:388-389`) even for top-level workflows. Net: one drive of an n-step workflow costs O(n²) store reads on JDBC, O(n³) event throughput on FDB. Beyond cost it produces the mixed-snapshot hazard (X9). Replaying from the pass-local snapshot fixes both.
...
| 15 | E6 | `intemporal.tests.engine.cascade-cancel-wake-test` | Cascade-cancel a timer-sleeping child, assert it's driven immediately (not stuck until original `wake-at`) on JDBC/InMemory. Also covers a second defect the E6 fix exposed: `replaying?` counted a bare `:timer-scheduled` as a resolved seq, so `check-cancelled!` deferred forever and cancellation was undeliverable to a workflow parked on `sleep` | Yes    |
...
| 28 | X8 | `intemporal.tests.crash.retry-durability-test`, `intemporal.tests.crash.async-retry-durability-test` | Crash mid-retry, resume, assert the attempt counter (not just the activity) survives across resumes; plus the backoff itself is durable (`:retry-at`, no drift, `wake-at`-gated) rather than a sleep on the drive thread, on both the sequential and async paths, and `timeout-ms` now bounds one attempt on both platforms | Yes |
...
- *A16's claim that `replaying?` scans the full history vector on every `next-seq!`* — **refuted while fixing #6**: `next-seq!` calls `check-cancelled!`, which reaches `surface-cancellation!` (the only caller of `replaying?`) **only when `p/is-cancelled?` returns true**. On a non-cancelled workflow `replaying?` never runs. The real per-seq cost in `next-seq!` is the `p/is-cancelled?` **store** read — which, after #6, is the only remaining per-op store read in the workflow body.
